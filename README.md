...
This Website is an E-commerce video platform which works as an extension of a real Yoga Studio, to offer Video Courses to their Students/Users who can't attend Physically to the In studio classes or the ones who prefer to learn and practise from home at different times. 

- ## The Business Goal

+ Increase the number of users.  
+ Increase the income of the Yoga studio and expands their number of customers from only In-studio customers to online customers.
...
Users will be able to create a Shopping cart to store Video-Courses prior to purchase
...
- ### User Stories
  ...
  - #### Returning Users Goals:
     ...
     - To Have a page with my information given such as username, email, and avatar.
...
- What makes a good experience? 
    * An easy-to-navigate website where they will never be more than a few clicks away from the important destinations 
    * A non-cluttered appearance to avoid over-irritable stimulation 
    * An easy purchasing process 
    * A consistent theme for the website 
    * Sufficient information regarding the direction to the product and non-direct information of the product such as categories and what the company is about 

- What we shouldn’t do? 
    * Make purchasing difficult as purchasing will be the main source of income 
    * Not registering should not have the same experience as registered users’ registered users should have a better experience in the form of offers, convenience etc 
...
Here is an explanation of the 3 mains colour of the palette.
...
Elegance, Modernism, and Sophistication.

Efficiency, Control, and Beauty.
...
In reality, grey invokes different feelings in different people. The various shades of grey can have a calming effect for some people, while it drives others into depression. It is safer to instil Gray in textiles rather than wall colours. If you do use it as a dominant room colour, make sure to introduce plenty of natural light to make the room feel more welcoming and warm.

Emotions associated with the colour Gray are:
...
The colour yellow is synonymous with sunshine, thereby spreading the effect of light and happiness. Yellow is also associated with intellect and prosperity, due to its close associations with the colour gold.

In interior design, the colour yellow is recommended for the kitchen, dining areas, hallways, and bathrooms. The colour automatically uplifts people’s spirits making the room feel bright and sunny.

Although most shades of yellow have a positive effect on the psyche, dull yellow colours instigate a feeling of doom, decay, and sickness. It is best to use yellow in its bright shades around the house. Make sure to use it sparingly though, as yellow stimulates uncontrollable emotions. Due to the extremely optimistic effect, a completely yellow room can drive up blood pressure. The brain associates high blood pressure with feelings of anger thus causing people to lose their temper without any provocation.
...
Emotions associated with the colour Yellow are:

Joy, Happiness, Care, Cordiality.

Intelligence, Excellence, Obedience, and Productivity.
...
Even If the use of a background Video is considered bad practice for UX I decided to go for it, to get the experience of implementing a video in one of my Projects and practising with embedded videos, I thought it was a good choice as the website is a Video Platform so can give the users an idea of what the sites offers, However the video it has an only little movement of the waves which is not noise at all.
...
the goal was to inspire a space where you can feel freedom, free of stress and where you can be able to breathe clean and fresh air.
...
- ## Features
 ...
 - #### Existing Features:
   ...
   - Navigation Menu: I decided to go with a simple and clean navigation bar to keep the feeling of a minimalist Yoga Studio which invites you to take a break and relax.
      
   ![features](static/images/readme/navbar.png "Navigation bar")

   - ##### Courses display list:

   - The users are able to see here all the courses that the platform offers. which name and price, the images are links which if the user purchases the course, redirect the users to the course content.

    ![features](static/images/Courselist.png "Course list")

   - ##### Course detail:

   - I decided to don’t use an auto-play video here as it was too noisy to navigate through the Content and be able to see all the sections of the courses, so I left the Video decoration around the course image to create the feeling of a video platform, every single image below in the section will open a video. 
    I did this by modifying the model and removing the url_intro_video. and adding the thumbnail image.   

   - Here the users will be able to see A course in detail, accessing all the content that the course offers will be composed of sections with video tutorials and series.
   ...
   - Here the Users will be able to view the videos in full screen, and also move through the different sections of the courses and the other videos using the dropdown list.
   ...
   - The users will be able to add, adjust quantity and remove Courses from the shopping cart once they register and Login
   - For the bag app I follow the videos of the Boutique-Ado, a side project from the Diploma in Code Institute, due to the complexity of the code and Stripe. I modify some bits to adapt to the requirements of This project

    ![features]( static/images/cart.png "Shopping cart")

   - ##### Checkout 

   - The users will be able to proceed with a secure payment, by completing the checkout form.
   - For the Checkout app I also follow the videos of the Boutique-Ado, a side project from the Diploma in Code Institute, due to the complexity of the code and Stripe. I modify some bits to adapt to the requirements of This project

    ![features]( static/images/checkout.png "Checkout")

   - ##### Registration and Login Form.

   - I also added a registration and Login Form, where users can Sign up and register to be able to modify the profile data to access the full content and purchase videos.
   ...
   - In the footer, the users can find all the contact detail, like social media links, phone numbers, and email, and I add a contact form using emailJS where the users can contact the business directly.
...
- ## Future Features I will like to include.
   ...
   - I will like to add the option to download the videos to the user's pc after purchase.

   - ##### Membership Options

   - I will probably include some membership options so the users can get access to specific content, depending on the membership they purchase.

   - In this project, the content can be accessed without actually being purchased, as I wanted to create the logic for a shopping cart using real-life e-commerce applications with the Idea that DVDs will be sent to the user's address without delivery cost.
  

   - ##### Yoga API

   - I will include a yoga API which will be used by users to translate some yoga poses from Sanskrit to their English names.
...
* REF003 - 'Content app' Was inspired and based on a side project I did in UDEMY It was Heavily modified as I change the functionality (https://www.udemy.com/course/draft/4651056/learn/lecture/31918044?start=270#overview)
...
## **DISCLAIMER**
This website was built for educational reasons for my coding course from Code Institute. No content, written(unless mentioned in CREDITS - Content of this document) and imagery, is of mine. No offence was intentionally made with the pricing of the Courses (I randomly made up the numbers, and also randomly collect the videos from YouTube as Content).
If there is anything you would like to enquire about, please do so via my email manuperezroomeroo@gmail.com
